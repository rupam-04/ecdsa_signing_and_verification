...
pub fn calculate_file_hash(path: &String) -> Result<String, ()> {
    let input: File = File::open(path).expect("file-open");
    let reader: BufReader<File> = BufReader::new(input);
    let digest = sha256_digest(reader).expect("sha256-digest");
...
pub fn get_private_key(private_key_file: &File) -> Option<String> {
    let buffered: BufReader<&File> = BufReader::new(private_key_file);
    
    for line in buffered.lines() {
        let content: String = line.expect("read-line");
...
pub fn get_public_key(public_key_file: &File) -> Option<(String, String)> {
    let buffered: BufReader<&File> = BufReader::new(public_key_file);
