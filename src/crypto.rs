...
pub mod secp256k1 {
    ...
    pub fn verify_signature(message: &str, signature: &str, public_key: &str) -> bool {
        let m: Message = Message::from_slice(&base16::decode_string(&message)).expect("conversion");
        let s: Signature = Signature::from_der(&base16::decode_string(signature)).expect("conversion");
        let pk: PublicKey = PublicKey::from_slice(&base16::decode_string(&public_key)).expect("conversion");
